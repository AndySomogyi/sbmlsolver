...
ffho(const iap_type *iap, const rap_type *rap, integer ndim, const doublereal *u, const doublereal *uold, const integer *icp, doublereal *par, doublereal *f, integer ndm, doublereal *dfdu)
{
  ...
  if (blhom_1.istart >= 0) {
    ...
    for (i = 0; i < ndm; ++i) {
      ...
      f[i + ndim - ndm] = par[sysoff+1] * f[i + ndim - ndm];
    }
  }
                        ...
                        const doublereal *xequib1, const doublereal *xequib2)
{
  ...
  if (blhom_1.itwist == 1 && blhom_1.istart >= 0) {
    ...
    for (k = 0; k < ndim/ndm-1; ++k) {
      for (i = 0; i < ndm; ++i) {
        ...
        if (blhom_1.itwist == 1) {
          ...
          /*     PAR(num_model_pars-2*NDM+1...num_model_pars-NDM) contains the adjoint unit */
          /*     vector at the gaps. */
          fb[jb] -= par[(k << 1) + 19] * par[num_total_pars - (ndm << 1) + i];
        }
        ++jb;
      }
    }
    /*     Poincare sections: <x-x_0,\dot x_0>=0                */
    /*     PAR(num_model_pars-NDM+1...num_model_pars) contain the derivatives of the     */
    ...
    for (k = 0; k < ndim/ndm-1; ++k) {
      fb[jb++] += fbho_ddotsub(&par[num_total_pars - ndm], &u1[k * ndm], umax, ndm);
    }
  }
  free_dmatrix(bound);
  free_dmatrix(vr[0]);
...
fbho(const iap_type *iap, integer ndim, doublereal *par, const integer *icp, integer nbc, integer nbc0, const doublereal *u0, const doublereal *u1, doublereal *fb)
{
  ...
  /*              write(9,*) 'Xequib:' */
  xequib1 = &par[sysoff+2];
    /*              write(9,*) I,XEQUIB1(I) */
  if (blhom_1.iequib >= 0) {
    xequib2 = &par[sysoff+2];
  } else {
    xequib2 = &par[ndm + 11];
  }
  ...
  if (nbcn > 0) {
    ...
    fprintf(fp6, "Evil BUG!: Negative number of boundary conditions left\n");
    throw "Evil BUG!: Negative number of boundary conditions left\n";
   // exit(1);
  }
  return 0;
} /* fbho_ */
...
inho(iap_type *iap, integer *icp, doublereal *par)
{
  ...
  blhmp_1.ipsi   = (integer *)malloc(sizeof(integer)*(num_total_pars));
  blhmp_1.ifixed = (integer *)malloc(sizeof(integer)*(num_total_pars));
...
/* Find maximum from the equilibrium */
  ...
  func(ndm, ups[jmax], icp, par, 0, &par[num_total_pars-ndm], &dum1, &dum2);

/*     PAR(num_model_pars-NDM+1...num_model_pars) contains the point furthest from */
/*     the equilibrium. */
...
/*     PAR(21,23,...) contain the times between Poincare sections */
  ...
  par[sysoff] = par[sysoff+1] * tmmax;
  par[19] = 0.;
  for (k = 1; k < ndim/ndm-1; ++k) {
    par[(k << 1) + 18] = par[sysoff+1];
    par[(k << 1) + 19] = 0.;
  }
  par[sysoff+1] *= 1. - tmmax;

/*     Remember adjoint at maximum for applying Lin's method */
/*     PAR(num_model_pars-2*NDM+1...num_model_pars-NDM) will contain the adjoint unit */
/*     vector at the gaps. */

  if (blhom_1.itwist == 1) {
    dnorm = 0.;
    for (i = 0; i < ndm; ++i) {
      par[num_total_pars - (ndm << 1) + i] = ups[jmax][ndm + i];
      dnorm += ups[jmax][ndm + i] * ups[jmax][ndm + i];
    }
    dnorm = sqrt(dnorm);
    for (i = 0; i < ndm; ++i) {
      par[num_total_pars - (ndm << 1) + i] /= dnorm;
    }
  }
...
/*     again once the branch switching is complete. */
{
  ...
  ncopy = *nar / ndm;
  j = 0;
  time = par[sysoff] + par[sysoff+1];
  for (k = 1; k < ncopy; ++k) {
    time += par[(k << 1) + 18];
  }
  tbase = time - par[sysoff+1];
  tm[*ntsr * ncopy] = 1.;
  for (k = ncopy - 1; k >= 0; --k) {
    for (j = *ntsr - 1; j >= 0; --j) {
      i = j + *ntsr * k;
      ...
      if (k == 0) {
        tm[i] = tm[j] * par[sysoff] / time;
      } else if (k == ncopy - 1) {
        tm[i] = (tbase + tm[j] * par[sysoff+1]) / time;
      } else {
...
*/
  ...
  for (i = 0; i < ndm; ++i) {
    u[i] = xequib[i] + vr[blhom_1.nstab][i] * par[kp] * par[kp + 1] *
      exp(rr[blhom_1.nstab] * t * par[sysoff+1]);
  }
  for (i = 0; i < ndm; ++i) {
  ...
  for (i = 0; i < blhom_1.nunstab; ++i) {
    ...
    for (j = 0; j < ndm; ++j) {
      par[ip + i + 1] += vr[blhom_1.nstab][j] * par[kp] * par[kp + 1] *
        exp(rr[blhom_1.nstab] * par[sysoff+1]) * vt[blhom_1.nstab + i][j];
    }
  }
  ip += blhom_1.nunstab;
  free(ri);
...
pvlsho(iap_type *iap, rap_type *rap, integer *icp, doublereal *dtm, integer *ndxloc, doublereal **ups, integer *ndim, doublereal **p0, doublereal **p1, doublereal *par)
{
  ...
  /*      *Compute eigenvalues */
  bleig.ineig = 0;
  eigho(1, 2, bleig.rr[0], bleig.ri[0], bleig.v[0], &par[sysoff+2], icp, par, ndm);
  if (blhom_1.iequib < 0) {
  ...
  for (i = 0; i < blhom_1.npsi; ++i) {
    if (blhmp_1.ipsi[i] > 10 && bleig.ineig == 0) {
      eigho(1, 1, bleig.rr[0], bleig.ri[0], bleig.vt[0], &par[sysoff+2], icp, par, ndm);
      if (blhom_1.iequib < 0) {
...
/* Local bifurcation (zero eigenvalue or Hopf): NSTAB increases */
 ...
 case 11:
  ...
  ret_val *= exp(-par[sysoff+1] * rr[0][blhom_1.nstab - 1] / 2.);
  break;
 ...
 case 12:
  ...
  ret_val *= exp(par[sysoff+1] * rr[0][blhom_1.nstab] / 2.);
  break;
...
eigho(integer isign, integer itrans, doublereal *rr, doublereal *ri, doublereal **vret, const doublereal *xequib, const integer *icp, doublereal *par, integer ndm)
{
  ...
  dfdp = (doublereal *)malloc(sizeof(doublereal)*ndm*num_total_pars);
