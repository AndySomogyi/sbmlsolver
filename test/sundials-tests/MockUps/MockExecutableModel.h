//
// Created by Ciaran on 18/02/2021.
//
 ...
 *
 * @author CMW
 * @brief Class to mock the executable model. We
 * do not want to need an actual model in some tests
 * as this introduces unwanted dependencies in a unit. Instead
 * when an ExecutableModel is needed inside a unit, we use the
 * mock object instead.
 */
...
public:
    ...
    MOCK_METHOD(int, getFloatingSpeciesConcentrationRates, (size_t len, int const *indx, double *values),
                (override));

    MOCK_METHOD(int, getFloatingSpeciesConcentrations, (size_t len, int const *indx, double *values), (override));

    MOCK_METHOD(int, setFloatingSpeciesConcentrations, (size_t len, int const *indx, double const *values),
                (override));

    MOCK_METHOD(int, setFloatingSpeciesInitConcentrations, (size_t len, int const *indx, double const *values),
                (override));

    MOCK_METHOD(int, getFloatingSpeciesInitConcentrations, (size_t len, int const *indx, double *values),
                (override));

    MOCK_METHOD(int, setFloatingSpeciesInitAmounts, (size_t len, int const *indx, double const *values),
                (override));
    ...
    MOCK_METHOD(int, setBoundarySpeciesConcentrations, (size_t len, int const *indx, double const *values),
                (override));
    ...
    MOCK_METHOD(int, applyEvents, (double timeEnd, const unsigned char *previousEventStatus,
