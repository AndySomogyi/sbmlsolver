...
# sbmlsolver. Herbert M Sauro November 2012
# Nov 2013: Modified to test Andy's SWIG API
#-------------------------------------------------------------

#------------------------------------------
# Change this line for different test files
#nameOfResultsFile = 'results_sbmlsolverTest_1.txt'


import random
import string
import sbmlsolver
from sbmlsolver import Config
from sbmlsolver import Logger
import re
import numpy
from numpy import *
...
def checkFullJacobian(rrInstance, testId):
    # Jacobian
    print string.ljust ("Check " + testId, rpadding),
    sbmlsolver.Config.setValue(Config.SBMLSOLVER_JACOBIAN_MODE, Config.SBMLSOLVER_JACOBIAN_MODE_CONCENTRATIONS)
...
def checkAmountJacobian(rrInstance, testId):
    # Jacobian
    print string.ljust ("Check " + testId, rpadding),
    sbmlsolver.Config.setValue(Config.SBMLSOLVER_JACOBIAN_MODE, Config.SBMLSOLVER_JACOBIAN_MODE_AMOUNTS)
    Jacobian = rrInstance.getFullJacobian()
    checkMatrixVsUpcomingText(Jacobian)
...
def checkJacobian(rrInstance, testId):
    ...
    # save the old value
    saved = Config.getValue(Config.SBMLSOLVER_JACOBIAN_MODE)

    # set to amounts mode
    Config.setValue(Config.SBMLSOLVER_JACOBIAN_MODE, Config.SBMLSOLVER_JACOBIAN_MODE_AMOUNTS)
    ...
    # set to conc mode
    Config.setValue(Config.SBMLSOLVER_JACOBIAN_MODE, Config.SBMLSOLVER_JACOBIAN_MODE_CONCENTRATIONS)
...
def runTester (testDir=None):
    ...
    for file in files:
        ...
        # create a SBMLSolver obj with the sbml from the test file
        rrInstance = sbmlsolver.SBMLSolver(sbmlStr)
