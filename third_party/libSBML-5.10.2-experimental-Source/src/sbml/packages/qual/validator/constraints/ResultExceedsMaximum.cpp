 ...
 */
ResultExceedsMaximum::ResultExceedsMaximum (unsigned int id, Validator& v) :
 ...
 */
ResultExceedsMaximum::~ResultExceedsMaximum ()
{
}
...
ResultExceedsMaximum::check_ (const Model& m, const Model& object)
{
  ...
  for (unsigned int i = 0; i < plug->getNumQualitativeSpecies(); i++)
  {
    ...
    {
      checkExceedsMaximum(plug->getQualitativeSpecies(i), plug);
    }
  }
...
void
ResultExceedsMaximum::checkExceedsMaximum(const QualitativeSpecies* qs, 
                                          const QualModelPlugin* plug)
{
  int maxLevel = qs->getMaxLevel();
  ...
  for (unsigned int i = 0; i < plug->getNumTransitions(); i++)
  {
    ...
    if (tr->isSetDefaultTerm())
    {
      ...
      {
        if (maxLevel < tr->getDefaultTerm()->getResultLevel())
        {
          logExceedsMaximum(*tr, *qs);
        }
      }
    }
    ...
    for (unsigned int j = 0; j < tr->getNumFunctionTerms(); j++)
    {
      ...
      {
        if (maxLevel < tr->getFunctionTerm(j)->getResultLevel())
        {
          logExceedsMaximum(*tr, *qs);
        }
      }
