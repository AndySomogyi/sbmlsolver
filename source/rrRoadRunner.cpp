...
string RoadRunner::getDLLName()
{
    ...
    string baseName = mModelLib.getFileName();
#if defined(_WIN32) || defined(__CODEGEARC__)
    string dllName  = JoinPath(tempFileFolder, ChangeFileExtensionTo(baseName, "dll"));
#else
	string dllName  = JoinPath(tempFileFolder, "lib" + ChangeFileExtensionTo(baseName, "so"));
#endif
    return dllName;
}
...
bool RoadRunner::generateModelCode(const string& sbml)
{
    ...
    if(!mModelGenerator->SaveSourceCodeToFolder(tempFileFolder, mModelLib.getFileName()))
    {
...
bool RoadRunner::compileModel()
{
    ...
    string dllName  = mModelLib.getFullFileName();
    ...
    try
    {
		Log(lDebug)<<"Loading shared lib: "<<dllName;
    	mModelLib.load();
    }
...
ModelFromC* RoadRunner::createModel()
{
    ...
    {
        CGenerator *codeGen = dynamic_cast<CGenerator*>(mModelGenerator);
        ModelFromC *rrCModel = new ModelFromC(codeGen, mModelLib);     //Todo: memoryleak
