...
bool RoadRunner::unLoadModelDLL()
{
    ...
    if(mModelDLL.isLoaded())	//Make sure the dll is unloaded
    {
	    mModelDLL.unload();
        return (!mModelDLL.isLoaded()) ? true : false;
    }
...
DoubleMatrix RoadRunner::simulate()
{
    try
    {
        if (!mModel)
        {
            throw Exception(gEmptyModelString);
        }
...
DoubleMatrix RoadRunner::simulateEx(const double& startTime, const double& endTime, const int& numberOfPoints)
{
    try
    {
        if (!mModel)
        {
            throw CoreException(gEmptyModelString);
        }
...
//        {
//            if (!mModel) throw CoreException(gEmptyModelString);
//
//            mModel->bc = values;
//        }
...
DoubleMatrix RoadRunner::getUnscaledFluxControlCoefficientMatrix()
{
	try
	{
		...
		else throw CoreException(gEmptyModelString);
	}
	catch (CoreException)
	{
		throw;
