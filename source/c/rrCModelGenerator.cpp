...
bool CModelGenerator::generateModelCode(const string& sbml, const string& modelName, bool computeAndAssignConsevationLaws)
{
    ...
    if(!modelCode.size())
    {
        Log(Logger::PRIO_ERROR)<<"Failed to generate model code";
        return false;
    }

    if(!saveSourceCodeToFolder(mTempFileFolder, modelName))
    {
        Log(Logger::PRIO_ERROR)<<"Failed saving generated source code";
    }
...
bool CModelGenerator::compileModel()
{
    ...
    {
        Log(Logger::PRIO_ERROR)<<"Failed compiling model";
        return false;
    }
...
bool CModelGenerator::compileCurrentModel()
{
    ...
    {
        Log(Logger::PRIO_ERROR)<<"Model failed compilation";
        return false;
    }
        ...
        bool forceReCompile, bool computeAndAssignConsevationLaws)
{
    ...
    try
    {
        ...
        if(!fileExists(mModelLib->getFullFileName()) || forceReCompile == true)
        {
            if(!compileModel())
            {
                Log(Logger::PRIO_ERROR)<<"Failed to generate and compile model";
                return 0;
            }

            if(!mModelLib->load())
            {
                Log(Logger::PRIO_ERROR)<<"Failed to load model DLL";
                return 0;
            }
        }
        else
    ...
    {
        Log(Logger::PRIO_ERROR)<<"Compiler problem: "<<ex.what();
    }
    ...
    {
        Log(Logger::PRIO_ERROR)<<"Failed to create model from DLL";
        model = NULL;
    }
