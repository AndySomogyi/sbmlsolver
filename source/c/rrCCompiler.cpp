...
bool CCompiler::setupCompiler(const string& supportCodeFolder)
{
    ...
    {
        Log(Logger::ERROR)<<"The roadrunner support code folder : "<<mSupportCodeFolder<<" does not exist.";
        return false;
    }
...
bool CCompiler::compileSource(const string& sourceFileName)
{
    ...
    if(!compile(exeCmd))
    {
        Log(Logger::ERROR)<<"Creating DLL failed..";
...
bool CCompiler::setCompilerLocation(const string& path)
{
    ...
    {
        Log(Logger::ERROR)<<"Tried to set invalid path: "<<path<<" for compiler location";
        return false;
    }
...
bool CCompiler::compile(const string& cmdLine)
{
                            ...
                            NULL))==INVALID_HANDLE_VALUE)
    {
        ...
        Log(Logger::ERROR)<<"WIN API Error (after CreateFile): "<<anError;
        Log(Logger::ERROR)<<"Failed creating logFile for compiler output";
    }
    ...
    )
    {
        ...
        Log(Logger::ERROR)<<"WIN API Error: (after CreateProcess) "<<anError;
...
bool CCompiler::compile(const string& cmdLine)
{
    ...
    {
        Log(Logger::ERROR)<<"Compile system call returned: "<<val;
        return false;
    }
}
