...
public:
     ...
     */
    EulerIntegrator(ExecutableModel *m, const SimulateOptions *o) {
        model = m;
        exampleParameter1 = 3.14;
        exampleParameter2 = "hello";
        setSimulateOptions(o);
    ...
    virtual void setSimulateOptions(const SimulateOptions* opt) {
        if(opt) {
            if(opt->hasKey("exampleParameter1")) {
                exampleParameter1 = opt->getItem("exampleParameter1");
            }

            if(opt->hasKey("exampleParameter2")) {
                exampleParameter2 = (string)opt->getItem("exampleParameter2");
            }
        }
    }
