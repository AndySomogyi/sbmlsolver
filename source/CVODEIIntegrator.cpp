...
namespace rr
{
	...
	int IcvodeDyDtFcn(realtype t, N_Vector cv_y, N_Vector cv_ydot, void *userData);
	int IcvodeRootFcn (realtype t, N_Vector y, realtype *gout, void *userData);
	...
	const int CVODEIIntegrator::mDefaultMaxNumSteps = 10000;
	const int CVODEIIntegrator::mDefaultMaxAdamsOrder = 12;
	const int CVODEIIntegrator::mDefaultMaxBDFOrder = 5;
	...
	void CVODEIIntegrator::setListener(IIntegratorListenerPtr p)
	{
		listener = p;
	}

	IIntegratorListenerPtr CVODEIIntegrator::getListener()
	{
		return listener;
	}

	CVODEIIntegrator::CVODEIIntegrator(ExecutableModel *aModel, const SimulateOptions* options)
	:
	mStateVector(NULL),
	mCVODE_Memory(NULL),
	lastEventTime(0),
	...
	options(*options)
	{
		Log(Logger::LOG_INFORMATION) << "creating CVODEIIntegrator";
	...
	CVODEIIntegrator::~CVODEIIntegrator()
	{
		delete[] variableStepPostEventState;
		freeCVode();
	}

	void CVODEIIntegrator::setSimulateOptions(const SimulateOptions* o)
	{
	...
	void CVODEIIntegrator::reInit(double t0)
	{
	...
	double CVODEIIntegrator::integrate(double timeStart, double hstep)
	{
		static const double epsilon = std::numeric_limits<double>::epsilon();

		Log(Logger::LOG_DEBUG) << "CVODEIIntegrator::integrate("
				<< timeStart <<", " << hstep << ")";
	...
	bool CVODEIIntegrator::haveVariables()
	{
		return stateVectorVariables;
	}

	void CVODEIIntegrator::createCVode()
	{
		if(!mModel)
		{
			return;
		...
		if ((err = CVodeInit(mCVODE_Memory, IcvodeDyDtFcn, t0, mStateVector)) != CV_SUCCESS)
		{
			handleCVODEError(err);
		}
	...
	void CVODEIIntegrator::testRootsAtInitialTime()
	{
	...
	void CVODEIIntegrator::applyEvents(double timeEnd, vector<unsigned char> &previousEventStatus)
	{
	...
	void CVODEIIntegrator::applyPendingEvents(double timeEnd)
	{
		if(mModel) {
	...
	void CVODEIIntegrator::assignResultsToModel()
	{
		if (mStateVector)
		{
	...
	void CVODEIIntegrator::setCVODETolerances()
	{
	...
	void CVODEIIntegrator::restart(double time)
	{
		if (!mModel) {
			return;
		}
	...
	int IcvodeDyDtFcn(realtype time, N_Vector cv_y, N_Vector cv_ydot, void *userData)
	{
		double* y = NV_DATA_S (cv_y);
		double* ydot = NV_DATA_S(cv_ydot);
		CVODEIIntegrator* cvInstance = (CVODEIIntegrator*) userData;
	...
	void CVODEIIntegrator::freeCVode()
	{
	...
	const Dictionary* CVODEIIntegrator::getIntegratorOptions()
	{
		// static instance
	...
	double CVODEIIntegrator::applyVariableStepPendingEvents()
	{
	...
	int IcvodeRootFcn (realtype time, N_Vector y_vector, realtype *gout, void *user_data)
	{
		CVODEIIntegrator* cvInstance = (CVODEIIntegrator*) user_data;
	...
	void CVODEIIntegrator::setItem(const std::string& key, const rr::Variant& value)
	{
		if (key == "BDFMaxOrder")
		{
	...
	Variant CVODEIIntegrator::getItem(const std::string& key) const
	{
		if (key == "BDFMaxOrder")
		{
			return mMaxBDFOrder;
	...
	bool CVODEIIntegrator::hasKey(const std::string& key) const
	{
		return key == "BDFMaxOrder" || key == "AdamsMaxOrder";
	}

	int CVODEIIntegrator::deleteItem(const std::string& key)
	{
		return -1;
	}

	std::vector<std::string> CVODEIIntegrator::getKeys() const
	{
	...
	std::string CVODEIIntegrator::toString() const
	{
		std::stringstream ss;
		ss << "< roadrunner.CVODEIIntegrator() " << endl << "{ "
	...
	std::string CVODEIIntegrator::toRepr() const
	{
		std::stringstream ss;
		ss << "< roadrunner.CVODEIIntegrator() { 'this' : "
				<< (void*)this << " }>";
		return ss.str();
	}

	std::string CVODEIIntegrator::getName() const
	{
		return "cvode";
	}
