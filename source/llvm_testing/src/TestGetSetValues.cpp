...
bool TestGetSetValues::test()
{
    ...
    Log(Logger::PRIO_INFORMATION) << "before set: " << model << endl;

    double val = 0;
    model->getCompartmentVolumes(1, 0, &val);

    Log(Logger::PRIO_INFORMATION) << "initial volume: " << val << endl;

    val = 2;
    model->setCompartmentVolumes(1, 0, &val);

    Log(Logger::PRIO_INFORMATION) << "after: " << model << endl;

    model->getCompartmentVolumes(1, 0, &val);

    Log(Logger::PRIO_INFORMATION) << "new volume: " << val << endl;

    double amt = 2;

    model->setFloatingSpeciesAmounts(1, 0, &amt);

    Log(Logger::PRIO_INFORMATION) << "after set amt: " << model << endl;

    model->setFloatingSpeciesConcentrations(1, 0, &amt);

    Log(Logger::PRIO_INFORMATION) << "after set conc: " << model << endl;
