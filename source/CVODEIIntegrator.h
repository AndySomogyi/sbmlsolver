 ...
 */
class CVODEIIntegrator : public IIntegrator
{
public:
    CVODEIIntegrator(ExecutableModel* oModel, const SimulateOptions* options);

    virtual ~CVODEIIntegrator();
     ...
     */
    virtual void setListener(IIntegratorListenerPtr);

    /**
     * get the integrator listener
     */
    virtual IIntegratorListenerPtr getListener();
...
private:
     ...
     * the listener
     */
    IIntegratorListenerPtr listener;
     ...
     * cvode dydt callback
     */
    friend int IcvodeDyDtFcn(double t, N_Vector cv_y, N_Vector cv_ydot, void *f_data);

    /**
     * cvode event root finding callback.
     */
    friend int IcvodeRootFcn (double t, N_Vector y, double *gout, void *g_data);
};
}
