...
SBMLSolverOptions::SBMLSolverOptions() :
    flags(0)
{
    if (Config::getBool(Config::SBMLSOLVER_DISABLE_PYTHON_DYNAMIC_PROPERTIES))
    {
        flags |= SBMLSolverOptions::DISABLE_PYTHON_DYNAMIC_PROPERTIES;
    }

    jacobianStepSize = Config::getDouble(Config::SBMLSOLVER_JACOBIAN_STEP_SIZE);
}
