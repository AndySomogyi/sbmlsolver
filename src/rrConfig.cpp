...
static Variant values[] =  {
    ...
    Variant(0),        // SBMLSOLVER_DISABLE_WARNINGS
    Variant(false),    // SBMLSOLVER_DISABLE_PYTHON_DYNAMIC_PROPERTIES
    Variant(int(AllChecksON & UnitsCheckOFF)),           //SBML_APPLICABLEVALIDATORS
    Variant(0.00001),   // SBMLSOLVER_JACOBIAN_STEP_SIZE
    ...
    Variant((int)Config::SBMLSOLVER_JACOBIAN_MODE_CONCENTRATIONS), // SBMLSOLVER_JACOBIAN_MODE
...
static void getKeyNames(StringIntMap& keys)
{
    ...
    keys["SBMLSOLVER_DISABLE_WARNINGS"] = rr::Config::SBMLSOLVER_DISABLE_WARNINGS;
    keys["SBMLSOLVER_DISABLE_PYTHON_DYNAMIC_PROPERTIES"] = rr::Config::SBMLSOLVER_DISABLE_PYTHON_DYNAMIC_PROPERTIES;
    keys["SBML_APPLICABLEVALIDATORS"] = rr::Config::SBML_APPLICABLEVALIDATORS;
    keys["SBMLSOLVER_JACOBIAN_STEP_SIZE"] = rr::Config::SBMLSOLVER_JACOBIAN_STEP_SIZE;
    ...
    keys["SBMLSOLVER_JACOBIAN_MODE"] = rr::Config::SBMLSOLVER_JACOBIAN_MODE;
...
{
    // check env var
    const char* env = std::getenv("SBMLSOLVER_CONFIG");
    std::string path;
    Poco::Path ppath;

    Log(rr::Logger::LOG_DEBUG) << "trying config file from SBMLSOLVER_CONFIG "
        << (env ? env : "NULL");
    ...
    ppath.setFileName("sbmlsolver.conf");
    path = ppath.toString();
