 ...
 *
 * Many of SBMLSolver classes use a number of configration parameters. Most of these can be set
 ...
 * the config file are:
 *
 * #1: the SBMLSOLVER_CONFIG environment variable
 *
 * #2: try the user's home directory for sbmlsolver.conf, i.e.:
 *
 * /Users/andy/sbmlsolver.conf
 *
 * #3: try the user's home directory for .sbmlsolver.conf, i.e.:
 *
 * /Users/andy/.sbmlsolver.conf
 *
 * #4: try the same directory as the sbmlsolver shared library, this
 * will be the same directory as the python _sbmlsolver.pyd python extension module, i.e.:
 *
 * /Users/andy/local/lib/sbmlsolver.conf
 *
 * #5: try one directory up from the where the shared library or program is at, i.e.:
 *
 * /Users/andy/local/sbmlsolver.conf
 *
...
public:
    ...
    enum Keys {
         ...
         * SBMLSolver to load a stiff solver which could potentially be
         * extremly slow
         ...
         * IntegratorListener set, SBMLSolver::integrate will run the
         ...
         *
         * Note, for each number of steps given to SBMLSolver::simulate or SBMLSolver::oneStep,
         ...
         *
         * SBMLSOLVER_DISABLE_WARNINGS_STEADYSTATE: disable the warning in
         * SBMLSolver::steadyState about possible singular jacobian.
         *
         * SBMLSOLVER_DISABLE_WARNINGS_CONSERVED_MOIETY: disable the
         * conserved moieties warning if enabled when loading a sbml document.
         */
        SBMLSOLVER_DISABLE_WARNINGS,

        /**
         * SBMLSolver by default dynamically generates accessor properties
         ...
         */
        SBMLSOLVER_DISABLE_PYTHON_DYNAMIC_PROPERTIES,
        ...
        /**
         * Determines the mode that the SBMLSolver Jacobian calculations will be
         * be performed in.  The Jacobian can be calculated either with respect to
         * amounts or concentrations. Default is concentrations.
         *
         * This can be either SBMLSOLVER_JACOBIAN_MODE_AMOUNTS or
         * SBMLSOLVER_JACOBIAN_MODE_CONCENTRATIONS.
         */
        SBMLSOLVER_JACOBIAN_MODE,

        /**
         * Directory path where sbmlsolver should store temp files. If empty, the
        ...
        /**
         * Path to where the logger should write a sbmlsolver log file. If no path
         * is specified, the log file will be written as a "sbmlsolver.log" in the
         * temp dir.
         *
         * May have environment variables contained in the path.
         * On Unix, a tilde as first character in the path is
    ...
    enum SBMLSOLVER_DISABLE_WARNINGS_VALUES
    {
        SBMLSOLVER_DISABLE_WARNINGS_STEADYSTATE          =  (0x1 << 0),  // => 0x00000001
        SBMLSOLVER_DISABLE_WARNINGS_CONSERVED_MOIETY     =  (0x1 << 1)   // => 0x00000010
    };


    enum SBMLSOLVER_JACOBIAN_MODE_VALUES
    {
        SBMLSOLVER_JACOBIAN_MODE_AMOUNTS = 0,
        SBMLSOLVER_JACOBIAN_MODE_CONCENTRATIONS
    };
