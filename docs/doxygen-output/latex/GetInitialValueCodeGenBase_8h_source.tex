\hypertarget{GetInitialValueCodeGenBase_8h_source}{}\doxysection{Get\+Initial\+Value\+Code\+Gen\+Base.\+h}
\label{GetInitialValueCodeGenBase_8h_source}\index{llvm/GetInitialValueCodeGenBase.h@{llvm/GetInitialValueCodeGenBase.h}}
...
\DoxyCodeLine{6 }
\DoxyCodeLine{7 \textcolor{preprocessor}{\#ifndef RRLLVM\_GETINITIALVALUECODEGENBASE\_H\_}}
\DoxyCodeLine{8 \textcolor{preprocessor}{\#define RRLLVM\_GETINITIALVALUECODEGENBASE\_H\_}}
\DoxyCodeLine{9 }
...
\DoxyCodeLine{23 \{}
\DoxyCodeLine{24 }
\DoxyCodeLine{25     \textcolor{keyword}{typedef} double (*GetInitialValueCodeGenBase\_FunctionPtr)(LLVMModelData*, size\_t);}
\DoxyCodeLine{26 }
\DoxyCodeLine{27     \textcolor{keyword}{template} <\textcolor{keyword}{typename} Derived, \textcolor{keywordtype}{bool} substanceUnits>}
\DoxyCodeLine{28     \textcolor{keyword}{class }\mbox{\hyperlink{classrrllvm_1_1GetInitialValueCodeGenBase}{GetInitialValueCodeGenBase}} :}
\DoxyCodeLine{29         \textcolor{keyword}{public} \mbox{\hyperlink{classrrllvm_1_1CodeGenBase}{CodeGenBase}}<GetInitialValueCodeGenBase\_FunctionPtr>}
\DoxyCodeLine{30     \{}
\DoxyCodeLine{31     \textcolor{keyword}{public}:}
\DoxyCodeLine{32         \mbox{\hyperlink{classrrllvm_1_1GetInitialValueCodeGenBase}{GetInitialValueCodeGenBase}}(\textcolor{keyword}{const} \mbox{\hyperlink{classrrllvm_1_1ModelGeneratorContext}{ModelGeneratorContext}}\& mgc);}
\DoxyCodeLine{33         \textcolor{keyword}{virtual} \mbox{\hyperlink{classrrllvm_1_1GetInitialValueCodeGenBase}{\string~GetInitialValueCodeGenBase}}();}
\DoxyCodeLine{34 }
\DoxyCodeLine{35         llvm::Value* codeGen();}
\DoxyCodeLine{36 }
\DoxyCodeLine{37         \textcolor{keyword}{typedef} GetInitialValueCodeGenBase\_FunctionPtr FunctionPtr;}
\DoxyCodeLine{38 }
\DoxyCodeLine{39     \};}
\DoxyCodeLine{40 }
\DoxyCodeLine{41     \textcolor{keyword}{template} <\textcolor{keyword}{typename} Derived, \textcolor{keywordtype}{bool} substanceUnits>}
\DoxyCodeLine{42     \mbox{\hyperlink{classrrllvm_1_1GetInitialValueCodeGenBase}{GetInitialValueCodeGenBase<Derived, substanceUnits>::GetInitialValueCodeGenBase}}(}
\DoxyCodeLine{43         \textcolor{keyword}{const} \mbox{\hyperlink{classrrllvm_1_1ModelGeneratorContext}{ModelGeneratorContext}}\& mgc) :}
\DoxyCodeLine{44         \mbox{\hyperlink{classrrllvm_1_1CodeGenBase}{CodeGenBase}}<GetInitialValueCodeGenBase\_FunctionPtr>(mgc)}
\DoxyCodeLine{45     \{}
\DoxyCodeLine{46     \}}
\DoxyCodeLine{47 }
\DoxyCodeLine{48     \textcolor{keyword}{template} <\textcolor{keyword}{typename} Derived, \textcolor{keywordtype}{bool} substanceUnits>}
\DoxyCodeLine{49     GetInitialValueCodeGenBase<Derived, substanceUnits>::\string~GetInitialValueCodeGenBase()}
\DoxyCodeLine{50     \{}
\DoxyCodeLine{51     \}}
\DoxyCodeLine{52 }
\DoxyCodeLine{53     \textcolor{keyword}{template} <\textcolor{keyword}{typename} Derived, \textcolor{keywordtype}{bool} substanceUnits>}
\DoxyCodeLine{54     llvm::Value* GetInitialValueCodeGenBase<Derived, substanceUnits>::codeGen()}
\DoxyCodeLine{55     \{}
...
\DoxyCodeLine{72 }
\DoxyCodeLine{73         ModelInitialValueSymbolResolver resolver(args[0], this-\/>modelGenContext);}
\DoxyCodeLine{74 }
\DoxyCodeLine{75         \textcolor{comment}{// default, return NaN}}
\DoxyCodeLine{76         llvm::BasicBlock* def = llvm::BasicBlock::Create(this-\/>context, \textcolor{stringliteral}{"{}default"{}}, this-\/>function);}
\DoxyCodeLine{77         this-\/>builder.SetInsertPoint(def);}
\DoxyCodeLine{78 }
\DoxyCodeLine{79         this-\/>builder.CreateRet(llvm::ConstantFP::get(this-\/>context,}
\DoxyCodeLine{80             llvm::APFloat::getQNaN(llvm::APFloat::IEEEdouble())));}
\DoxyCodeLine{81 }
\DoxyCodeLine{82         \textcolor{comment}{// write the switch at the func entry point, the switch is also the}}
\DoxyCodeLine{83         \textcolor{comment}{// entry block terminator}}
\DoxyCodeLine{84         this-\/>builder.SetInsertPoint(entry);}
\DoxyCodeLine{85 }
\DoxyCodeLine{86         llvm::SwitchInst* s = this-\/>builder.CreateSwitch(args[1], def, \textcolor{keyword}{static\_cast<}\textcolor{keywordtype}{unsigned} \textcolor{keywordtype}{int}\textcolor{keyword}{>}(ids.size()));}
\DoxyCodeLine{87 }
\DoxyCodeLine{88         \textcolor{keywordflow}{for} (\textcolor{keywordtype}{int} i = 0; i < ids.size(); ++i)}
\DoxyCodeLine{89         \{}
\DoxyCodeLine{90             llvm::BasicBlock* block = llvm::BasicBlock::Create(this-\/>context, ids[i] + \textcolor{stringliteral}{"{}\_block"{}}, this-\/>function);}
\DoxyCodeLine{91             this-\/>builder.SetInsertPoint(block);}
\DoxyCodeLine{92 }
...
\DoxyCodeLine{98             \textcolor{keyword}{const} libsbml::SBase* sbase = \textcolor{keyword}{const\_cast<}libsbml::Model*\textcolor{keyword}{>}(model)-\/>getElementBySId(ids[i]);}
...
\DoxyCodeLine{140 }
\DoxyCodeLine{141 }
\DoxyCodeLine{142 \textcolor{preprocessor}{\#endif }\textcolor{comment}{/* RRLLVM\_GETINITIALVALUECODEGENBASE\_H\_ */}\textcolor{preprocessor}{}}
