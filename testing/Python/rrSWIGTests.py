...
def checkIndividualEigenvalues(rrInstance, testId):
   ...
   try:
     for i in range(0,m):
        ...
        realPart = rrInstance.getSelectionValue ('eigen(' + eigenvalueName + ')')
...
def checkFloatingSpeciesInitialConditionIds (rrInstance, testId):
  ...
  line = readLine ()
  words = line.split()
  expected = rrInstance.getFloatingSpeciesInitConditionIds()
  m = rrInstance.model.getNumFloatingSpecies()
  for i in range(0,m):
      if words[i] != expected[i]:
             errorFlag = True
...
def runTester (pathToModels, testModel):
    ...
    #print 'Version: ', rrInstance.getVersion()
    #print 'Build Date: ', rrInstance.getBuildDate()
    #print 'Copyright Message: ', rrInstance.getCopyright()
    ...
    if testId == '[START_TESTS]':
        ...
        while testId != '[END_TESTS]':
           if functions.has_key(testId):
              func = functions[testId]
              func(rrInstance, testId)
           else:getFloatingSpeciesAmountRates
